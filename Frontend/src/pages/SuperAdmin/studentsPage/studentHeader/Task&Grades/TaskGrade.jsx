...
const TaskGrade = ({ studentId }) => {
  ...
  return (
    <div style={styles.container} ref={topRef}>
      {!hasData ? (
        <div style={styles.noRecords}>
          ...
          <p style={{ maxWidth: '300px', fontSize: '14px', lineHeight: '1.6' }}>
            You haven't enrolled in any workshops yet. Check back later for your progress logs.
          </p>
        </div>
      ) : (
        <>
          ...
          <div style={styles.headerCard}>
            ...
            <h1 style={{ margin: '0 0 24px 0', fontSize: '28px', fontWeight: '900' }}>
              {viewingWorkshop?.title || 'Workshop Detail'}
            </h1>
            <div style={{ display: 'flex', gap: '40px', flexWrap: 'wrap' }}>
              <div>
                <div style={{ fontSize: '10px', color: '#94a3b8', fontWeight: '900', marginBottom: '6px' }}>FACULTY</div>
            ...
            {isShowingCurrent && (
              <div style={styles.badgeDay}>
                CURRENT DAY
                <br />
                <span style={{ fontSize: '16px', fontWeight: '900' }}>{currentDayInfo}</span>
              </div>
            )}
          </div>
          ...
          <h3 style={styles.sectionTitle}>Daily Performance Log</h3>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(320px, 1fr))', gap: '24px' }}>
            {viewingWorkshop?.tasks
              .slice(performancePage * PERF_LIMIT, (performancePage + 1) * PERF_LIMIT)
              .map((task) => {
                const isActive = task.date === selectedDate;
                const isLocked = task.status === 'Upcoming';
                return (
                  <div key={task.day} style={styles.perfCard(isActive, isLocked)}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '20px', alignItems: 'center' }}>
                      ...
                      {task.points > 0 ? (
                        ...
                        >
                          <Icons.Clock /> {isLocked ? 'Locked' : 'Active'}
                        </span>
                      )}
                    </div>
                    <div style={{ fontWeight: '800', fontSize: '18px', marginBottom: '25px', lineHeight: '1.4' }}>
                      {task.title}
                    </div>
                    <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                      <div>
                        ...
                        <div
                          style={{
                            fontWeight: '900',
                            fontSize: '18px',
                            color: task.grade === 'A+' ? '#059669' : '#2563eb',
                          }}
                        >
                          {task.grade}
                        </div>
          ...
          {viewingWorkshop?.tasks.length > PERF_LIMIT && (
            ...
            >
              ...
              <div style={{ display: 'flex', gap: '12px' }}>
                ...
                <button
                  style={styles.navBtn}
                  disabled={(performancePage + 1) * PERF_LIMIT >= viewingWorkshop.tasks.length}
                  onClick={() => setPerformancePage((p) => p + 1)}
                >
                  Next
                </button>
      ...
      <div>
        {studentData.history.slice(historyPage * HIST_LIMIT, (historyPage + 1) * HIST_LIMIT).map((item) => (
          <div
            ...
            onClick={() => handleWorkshopSelection(item)}
          >
            <div>
              <div style={{ fontWeight: '900', fontSize: '16px', marginBottom: '4px' }}>{item.title}</div>
              <div style={{ fontSize: '13px', color: '#64748b', fontWeight: '600' }}>
                {item.date} • {item.duration} • {item.faculty}
              </div>
            </div>
            <div
              style={{
      ...
      >
        <span>
          Showing {historyPage * HIST_LIMIT + 1}-{Math.min((historyPage + 1) * HIST_LIMIT, studentData.history.length)}{' '}
          of {studentData.history.length} events
        </span>
        <div style={{ display: 'flex', gap: '12px' }}>
          <button style={styles.navBtn} disabled={historyPage === 0} onClick={() => setHistoryPage((p) => p - 1)}>
            Prev
          </button>
          <button
            style={styles.navBtn}
            disabled={(historyPage + 1) * HIST_LIMIT >= studentData.history.length}
