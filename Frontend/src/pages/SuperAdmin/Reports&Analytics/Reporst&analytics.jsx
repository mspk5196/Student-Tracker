...
const ReportsAnalytics = () => {
    ...
    useEffect(() => {
        const fetchTasks = async () => {
            ...
            try {
                const response = await fetch(`${API_URL}/tasks/venue/${selectedVenueId}`, {
                    headers: {
                        'Authorization': `Bearer ${token}`,
                        'Content-Type': 'application/json'
                    }
            ...
            try {
                ...
                if (data.success) {
                    const transformed = data.data.map(sub => {
                        const hasFile = sub.file_path && sub.file_name && sub.file_name !== 'Link Submission';
                        const hasLink = sub.link_url || (sub.file_path && (sub.file_path.startsWith('http://') || sub.file_path.startsWith('https://')));
                        ...
                        return {
                            ...
                            date: sub.submitted_at ? formatDate(sub.submitted_at) : 'Not submitted',
                            file: hasFile ? (sub.file_name || 'File') : null,
                            fileUrl: hasFile ? sub.file_path : null,
                            link: sub.link_url || (hasLink && !hasFile ? sub.file_path : null),
                            status: sub.status,
    ...
    const filteredSubmissions = useMemo(() => {
        return submissions.filter(item => {
            ...
            const matchesStatus = filters.status === 'All Statuses' || 
                (filters.status === 'Not Submitted' ? !item.hasSubmitted : item.status === filters.status);
    ...
    const handleExport = () => {
        const csvContent = "data:text/csv;charset=utf-8,"
            ...
            + filteredSubmissions.map(e => `${e.id},${e.name},${e.date},${e.file || 'N/A'},${e.link || 'N/A'},${e.status},${e.grade}`).join("\n");
    ...
    return (
        ...
        `}</style>
            ...
            {/* --- HEADER WITH VENUE & TASK SELECTION --- */}
            ...
            {!selectedVenueId || !selectedTaskId ? (
                <div className="table-wrap">
                    <div className="empty-state">
                        ...
                        <p>Please select a venue and task from the dropdowns above to view submissions.</p>
                    </div>
                </div>
            ) : (
                <>
            ...
            <div className="stats-grid">
                <StatCard
                    ...
                    value={stats.total}
                    trend={<><ArrowUpwardIcon sx={{ fontSize: 16 }} /> In this task</>}
                    color="trend-up"
                />
                <StatCard
                ...
                <StatCard
                    ...
                    trend={<><ArrowUpwardIcon sx={{ fontSize: 16 }} /> Graded submissions</>}
                    color="trend-up"
                />
                <StatCard
                    title="Late Submissions"
                    value={stats.late}
                    trend={<><ArrowDownwardIcon sx={{ fontSize: 16 }} /> After due date</>}
                    color="trend-down"
                />
            </div>
            ...
            <div className="toolbar">
                <div className="filters">
                    ...
                    >
                        ...
                        <MenuItem onClick={() => { setFilters({ ...filters, status: 'Not Submitted' }); setAnchorElStatus(null); }}>Not Submitted</MenuItem>
                    </Menu>
                </div>
                <div className="filters">
                        ...
                        {loading ? (
                            ...
                            currentItems.map((student) => (
                                <tr key={student.id}>
                                    ...
                                    <td>
                                        <div className={`status-tag ${!student.hasSubmitted ? 'tag-not-submitted' : 
                                                student.status === 'Graded' ? 'tag-graded' : 
                                                student.status === 'Needs Revision' ? 'tag-revision' : 'tag-pending'
                                            }`}>
                                            {student.hasSubmitted ? student.status : 'Not Submitted'}
                                        </div>
                                    </td>
                                    <td>
                                        <div className="mark-pill">
                                            <input
                                                ...
                                                placeholder="--"
                                                disabled={!student.hasSubmitted || (student.status === 'Graded' && student.action !== 'Update' && student.action !== 'Save' && student.action === 'Edit')}
                                            />
